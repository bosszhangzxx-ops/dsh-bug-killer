...
4. DSH 先识别项目类型、启动方式和日志配置，再定位相关功能的完整方法链并添加临时埋点；随后按提示启动或重启项目。
5. 点击 **开始复现**。插件记录日志文件当前字节位置。
6. 去业务页面完整复现问题。
7. 返回 DSH 点击 **已复现**。插件只读取开始之后新增的日志，并自动把带安全约束的修复任务发送给 DSH。
8. DSH 根据证据修复代码、运行检查，并按照本次追踪标识自动删除临时埋点和临时日志配置。

插件本身不直接修改业务代码；用户点击“开始追踪”和“已复现”后，它会分别自动提交埋点任务和修复任务，由 DSH 执行并汇报代码改动。
...
> 独立开发 DeepSeek Harness 调试插件 dsh-bug-killer，基于 Web UI Slot 与本机 RPC 打通浏览器交互和宿主文件能力；实现技术栈识别、完整业务链路埋点、字节偏移日志采集、轮转检测、敏感信息脱敏和工作区路径隔离，形成“埋点—复现—证据—Agent 修复—自动清理”的闭环，并补齐严格类型检查、自动化测试与 CI。
