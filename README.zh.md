...
2. 填写问题、预期结果、实际结果和 Spring 日志文件。
3. 点击 **开始追踪**。插件把“添加临时业务埋点”的提示词放入输入框，由你确认发送。
4. 等待 DSH 完成埋点，重启本地 Spring 服务。
5. 点击 **开始复现**。插件记录日志文件当前字节位置。
6. 去业务页面完整复现问题。
7. 返回 DSH 点击 **已复现**。插件只读取开始之后新增的日志，并生成带安全约束的排障提示词。
8. 检查输入框内容后手动发送，让 DSH 定位并修复代码。

插件不会自动发送消息，也不会自己修改业务代码。

## 能力

- DSH Web 输入框右侧原生按钮和四阶段对话框。
...
- Spring 服务必须把日志写入当前 DSH 工作区中的文件

> Bug Killer 不能直接读取 IntelliJ IDEA 的 Run/Debug 控制台。若项目只有控制台输出，请先配置文件日志。
...
- 点击“开始复现”之前应完成埋点并重启服务，否则记录的日志起点不准确。
...
> 独立开发 DeepSeek Harness 调试插件 dsh-bug-killer，基于 Web UI Slot 与本机 RPC 打通浏览器交互和宿主文件能力；通过字节偏移、轮转检测、日志截断、敏感信息脱敏和工作区路径隔离，实现 Spring 业务问题“埋点—复现—日志证据—Agent 修复”的可追溯闭环，并补齐严格类型检查、自动化测试与 CI。
