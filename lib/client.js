...
${evidence}
			...
			return function BugKillerButton(props) {
				...
				react.default.useEffect(() => {
					if (stored.stage === "instrumenting") {
						...
						if (!props.session.running && instrumentationRan.current) {
							...
							setStored((current) => current.stage === "instrumenting" ? props.session.promptError == null ? {
								...current,
								stage: "instrumented"
							} : {
								...current,
				...
				const startReproduction = async () => {
					...
					try {
						...
						const logPath = found.some((log) => log.relativePath === stored.logPath) ? stored.logPath : found[0]?.relativePath ?? stored.logPath;
		...
		function renderBody(h, state, patch, options) {
			...
			if (state.stage === "instrumented") return h("div", { className: "dbk-grid" }, h("div", { className: "dbk-card" }, h("h3", { className: "dbk-card-title" }, "埋点已经完成"), h("p", null, "按照 DSH 给出的方式启动或重启项目。确认日志文件开始产生内容后，点击“开始复现”。")), summaryCard(h, state));
			if (state.stage === "fixing") return h("div", { className: "dbk-card" }, h("div", { className: "dbk-live-row" }, h("span", { className: "dbk-dot dbk-dot-live" }), options.running ? "DSH 正在根据日志定位并修复" : "正在提交日志证据"), h("p", null, "修复任务已经自动发送。完成修复和检查后，DSH 会自动删除本次临时埋点与临时日志配置。"));
			if (state.stage === "complete") return h("div", { className: "dbk-card" }, h("h3", { className: "dbk-card-title" }, "本次修复任务已完成"), h("p", null, "DSH 已结束修复流程，并按任务要求检查和清理本次追踪标识对应的临时埋点。你可以查看会话中的修复结果。"), summaryCard(h, state));
			if (state.stage === "failed") return h("div", { className: "dbk-card" }, h("h3", { className: "dbk-card-title" }, state.failedTask === "diagnosis" ? "修复任务未正常完成" : "埋点任务未正常完成"), h("p", null, state.failedTask === "diagnosis" ? "请查看 DSH 会话里的错误。任务中断时临时埋点可能尚未清理，接受代码前请搜索本次追踪标识。" : "请查看 DSH 会话里的错误。本次追踪没有进入复现阶段。"), summaryCard(h, state));
			...
			}, "更改")), h("p", { className: "dbk-help" }, "只能选择当前 DSH 工作目录本身或它下面的子目录。")), options.directoryPickerOpen && options.directoryListing !== void 0 ? directoryPicker(h, options.directoryListing, options) : null);
		}
		function directoryPicker(h, listing, actions) {
			return h("div", { className: "dbk-directory-picker" }, h("p", {
			...
			}, `▸ ${directory.name}`)), listing.directories.length === 0 ? h("p", { className: "dbk-help" }, "这个目录下没有可选的子目录。") : null), h("div", { className: "dbk-directory-actions" }, h("button", {
				type: "button",
				className: "dbk-button",
				onClick: actions.closeDirectoryPicker
			}, "取消"), h("button", {
				type: "button",
				className: "dbk-button dbk-button-primary",
		...
		function renderFooter(h, stage, busy, actions) {
			...
			let right;
			if (stage === "setup") right = [button("取消", actions.close), button(busy ? "正在准备…" : "开始追踪", () => {
				actions.startTracking();
			}, "primary")];
			else if (stage === "instrumenting") right = [button("关闭", actions.close)];
			else if (stage === "instrumented") right = [button("修改问题", actions.edit), button(busy ? "正在检查日志…" : "开始复现", () => {
			...
			else if (stage === "failed") right = [button("关闭", actions.close), button("追踪新问题", () => {
				actions.reset();
			}, "primary")];
		...
		function isStoredState(value) {
			...
			return typeof candidate.issue === "string" && typeof candidate.projectPath === "string" && typeof candidate.logPath === "string" && typeof candidate.traceId === "string" && (candidate.stage === "setup" || candidate.stage === "instrumenting" || candidate.stage === "instrumented" || candidate.stage === "capturing" || candidate.stage === "fixing" || candidate.stage === "complete" || candidate.stage === "failed") && (candidate.failedTask === void 0 || candidate.failedTask === "instrumentation" || candidate.failedTask === "diagnosis") && (candidate.startedAt === void 0 || typeof candidate.startedAt === "number");
		}
