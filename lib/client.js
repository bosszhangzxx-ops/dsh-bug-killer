...
5. 执行过程中不要输出计划、文件列表或清理报告。最终只回答“临时日志埋点已清理。”，不要增加其他内容。`;
		...
		function createBugKillerButton(connection) {
			return function BugKillerButton(props) {
				...
				const triggerShell = h("div", { className: "dbk-trigger-shell" }, h("button", {
					type: "button",
				...
				}, h("span", { className: `dbk-dot${statusLive ? " dbk-dot-live" : ""}` }), h("span", null, `Bug Killer${statusLabel === "" ? "" : ` · ${statusLabel}`}`)), stored.stage === "awaitingResolution" ? h("div", {
					className: "dbk-result-drawer",
					role: "dialog",
					"aria-label": "确认问题是否解决"
				}, h("p", { className: "dbk-result-question" }, "刚才的问题解决了吗？"), error === "" ? null : h("p", {
					className: "dbk-result-error",
					role: "alert"
				}, error), h("div", { className: "dbk-result-actions" }, h("button", {
					type: "button",
					className: "dbk-button",
					onClick: confirmUnresolved
				}, "未解决"), h("button", {
					type: "button",
					className: "dbk-button dbk-button-primary",
					onClick: confirmResolved
				}, "已解决"))) : stored.stage === "cleaning" ? h("div", {
					className: "dbk-result-drawer",
					role: "status"
				}, h("p", { className: "dbk-result-question" }, "正在清理本次临时日志埋点…")) : null);
				...
				return h(react.default.Fragment, null, triggerShell, modal);
			};
		}
		function renderBody(h, state, patch, options) {
			...
			if (state.stage === "awaitingResolution") return h("div", { className: "dbk-card" }, h("h3", { className: "dbk-card-title" }, "请确认刚才的问题是否解决"), h("p", null, "请使用 Bug Killer 按钮下方的“已解决”或“未解决”继续。"));
		...
		function renderFooter(h, stage, failedTask, busy, actions) {
			...
			else if (stage === "awaitingResolution" || stage === "cleaning") right = [button("关闭", actions.close)];
