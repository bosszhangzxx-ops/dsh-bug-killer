...
describe('Bug Killer browser flow', () => {
  ...
  it('walks from issue form to capture and puts guarded evidence in the composer', async () => {
    ...
    props.session.running = false
    view.rerender(React.createElement(Component, props))
    const pendingTrigger = await screen.findByRole('button', { name: /Bug Killer · 待确认/ })
    expect(pendingTrigger.className).toContain('dbk-trigger-attention')
    expect(screen.queryByRole('dialog')).toBeNull()
    expect(screen.queryByText('请确认刚才的问题是否解决')).toBeNull()
    fireEvent.click(pendingTrigger)
    expect(screen.getByText('请确认刚才的问题是否解决')).toBeTruthy()
  ...
  it('cleans instrumentation and resets only after the user confirms the issue is solved', async () => {
    ...
    expect(screen.queryByRole('dialog')).toBeNull()
    expect(screen.queryByText('请确认刚才的问题是否解决')).toBeNull()
    const pendingTrigger = screen.getByRole('button', { name: /Bug Killer · 待确认/ })
    expect(pendingTrigger.className).toContain('dbk-trigger-attention')
    fireEvent.click(pendingTrigger)
    expect(screen.getByText('请确认刚才的问题是否解决')).toBeTruthy()
    const resolvedButton = screen.getByRole('button', { name: '已解决，并删除埋点日志' })
    expect(resolvedButton.className).toContain('dbk-button-primary')
